...
| 47 | The file fallback's 0600 protects the refresh token on every platform | **First CI run on Windows, 2026-09-16** | **Refuted, and the warning was asserting it.** Go's file modes do not map to Windows ACLs: the token file is written with 0600 and lands at **0666**, readable by any account on the machine. Two tests caught it, having never run there. The defect is not the tests — it is that both warnings named the protection in their text, "mode 0600", on every read and every save. A sentence a user would rely on, false on one of the three supported platforms. `FileProtection()` says what the platform actually gives, and on Windows says the file is not restricted at all. This is the same failure as promising `none` means silence (§4.3 rule 3): claiming a guarantee the platform declines to make. **Open, and the maintainer's:** whether Windows should refuse the plaintext fallback outright rather than warn. Refusing leaves a Windows user with no fallback when the keyring is unavailable; warning leaves an unprotected token on disk. §10 chose warn when it believed the file was protected, and that belief is now gone |
